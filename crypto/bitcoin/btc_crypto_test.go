...
func tGenKey() (*btc.PrivateKey, *ecdsa.PublicKey, error) {

	privKey, pubKey, err := genKey()
	if err != nil {
		return nil, nil, err
	}
	return privKey.(*btc.PrivateKey), pubKey.(*ecdsa.PublicKey), nil
}
...
func TestS2PKSignature(t *testing.T) {
	...
	content := "hello world"
	sig1, err := E.Sign([]byte(content), &crypto.PrivateKey{Source: crypto.BTC, SigType: crypto.Signature2PublicKey, PriKey: pk.D})
	if err != nil {
		t.Errorf("sign fail, err : %s", err)
	}

	sig2, err := E.Sign([]byte(content), &crypto.PrivateKey{Source: crypto.BTC, SigType: crypto.Signature2PublicKey, PriKey: pk.D.Bytes()})
	if err != nil {
		t.Errorf("sign fail, err : %s", err)
	}

	sig3, err := E.Sign([]byte(content), &crypto.PrivateKey{Source: crypto.BTC, SigType: crypto.Signature2PublicKey, PriKey: pk})
	if err != nil {
		t.Errorf("sign fail, err : %s", err)
	}

	if sig1.Source != sig2.Source || sig1.Source != sig3.Source || sig1.Source != crypto.BTC {
		t.Errorf("signature source should be %s", crypto.BTC)
	}
...
func TestS2PKVerify(t *testing.T) {
	...
	content := "hello world"
	sig, _ := E.Sign([]byte(content), &crypto.PrivateKey{Source: crypto.BTC, SigType: crypto.Signature2PublicKey, PriKey: pk.D})

	verify1, err := E.Verify([]byte(content), &crypto.Signature{
		PublicKey: crypto.PublicKey{Source: crypto.BTC, SigType: crypto.Signature2PublicKey, PubKey: pk.PublicKey}, Signature: sig.Signature})
	if err != nil {
	...
	verify2, err := E.Verify([]byte(content), &crypto.Signature{PublicKey: crypto.PublicKey{Source: crypto.BTC,
	...
	verify3, err := E.Verify([]byte(content), &crypto.Signature{PublicKey: crypto.PublicKey{Source: crypto.BTC,
...
func TestMSSignature(t *testing.T) {
	...
	content := "hello world"
	var pks []interface{}
	sig1, err := E.Sign([]byte(content), &crypto.PrivateKey{Source: crypto.BTC, SigType: crypto.MultipleSignatures, PriKey: append(pks, pk1.D, pk2.D, pk3.D)})
	if err != nil {
		t.Errorf("sign fail, err : %s", err)
	}
	pks = []interface{}{}
	sig2, err := E.Sign([]byte(content), &crypto.PrivateKey{Source: crypto.BTC, SigType: crypto.MultipleSignatures, PriKey: append(pks, pk1.D.Bytes(), pk2.D.Bytes(), pk3.D.Bytes())})
	if err != nil {
		t.Errorf("sign fail, err : %s", err)
	}
	pks = []interface{}{}
	sig3, err := E.Sign([]byte(content), &crypto.PrivateKey{Source: crypto.BTC, SigType: crypto.MultipleSignatures, PriKey: append(pks, pk1, pk2, pk3)})
	if err != nil {
		t.Errorf("sign fail, err : %s", err)
	}
...
func TestMSVerify(t *testing.T) {
	...
	content := "hello world"
	var pks []interface{}
	sig, _ := E.Sign([]byte(content), &crypto.PrivateKey{Source: crypto.BTC, SigType: crypto.MultipleSignatures, PriKey: append(pks, pk1.D, pk2.D, pk3.D)})

	var pubks []interface{}
	verify1, err := E.Verify([]byte(content), &crypto.Signature{PublicKey: crypto.PublicKey{Source: crypto.BTC,
	...
	pubKeyBytes1 = fromECDSAPub(&pk1.PublicKey)
	pubKeyBytes3 = fromECDSAPub(&pk3.PublicKey)
	_, err = E.Verify([]byte(content), &crypto.Signature{PublicKey: crypto.PublicKey{Source: crypto.BTC,
	...
	pubKeyBytes2 = fromECDSAPub(&pk2.PublicKey)
	pubKeyBytes3 = fromECDSAPub(&pk3.PublicKey)
	verify4, err := E.Verify([]byte(content), &crypto.Signature{PublicKey: crypto.PublicKey{Source: crypto.BTC,
